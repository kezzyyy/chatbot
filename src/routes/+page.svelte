<script lang="ts">
  import MarkdownIt from "markdown-it";
  import { onMount } from "svelte";

  let response = "";
  let chat = "";
  let loading = false;
  let error = "";
  const md = new MarkdownIt();


  $: renderedResponse = md.render(response);

  onMount(() => {
    console.log("Component mounted!");
  });

const onSubmit = async () => {
  if (!chat.trim()) return;

  loading = true;
  error = "";
  console.log("Sending request with chat:", chat);

    try {
      const res = await fetch("http://localhost:5173/api/chat", { 
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ chat })
      });

    if (!res.ok) {
      throw new Error(`API responded with error: ${res.statusText}`);
    }


    const resData = await res.json();
    console.log("API Raw Response:", resData);

    if (resData.error) {
      error = `${resData.error}${resData.details ? `: ${resData.details}` : ''}`;
      console.error("API Error:", error);
    } else {
      response = resData.message?.content || "No response from AI";
    }
  } catch (err) {
    console.error("Error fetching response:", err);
    error = "Failed to connect to the server. Please try again.";
  } finally {
    loading = false;
  }
};

</script>

<div class="min-h-screen flex flex-col items-center justify-center p-6" style="background-image: url('https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExdDJndjE0bjRuZTY5eHdjczB5ajk2N3ZpMTVua3M5NHE5NThvYjhoMiZlcD12MV9pbnRlcm5naWZfYnlfaWQmY3Q9Zw/xT0GqcCJJJH12hJvGM/giphy.gif'); background-size: cover; background-position: center; background-repeat: no-repeat;">
  <div class="max-w-3xl w-full bg-white/80 rounded-xl shadow-xl p-8 border border-blue-300 backdrop-blur-lg flex flex-col-reverse md:flex-row items-center">
    <div class="md:w-1/2 w-full text-center md:text-left p-4">
      <h2 class="text-4xl font-bold text-blue-500 mb-4 uppercase tracking-wide">
        Chat Assistant
      </h2>
      <p class="text-blue-400 text-md mb-6 italic">With my informarion</p>
    </div>
    <div class="md:w-1/2 w-full space-y-4 p-4">
      <textarea 
        bind:value={chat} 
        placeholder="Type your question..." 
        class="w-full h-24 resize-none rounded-md border border-blue-400 p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-300 placeholder-blue-400 bg-white/80 shadow-md"
        on:keydown={(e) => e.key === 'Enter' && !e.shiftKey && onSubmit()}
      ></textarea>
      
      <button 
        on:click={onSubmit}
        disabled={loading} 
        class="w-full rounded-md bg-gradient-to-r from-blue-400 to-blue-600 p-3 text-white font-semibold hover:from-blue-500 hover:to-blue-700 transform hover:scale-110 transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {#if loading}
          <span class="inline-block animate-pulse">Loading...</span>
        {:else}
          Ask Now
        {/if}
      </button>
      
      {#if error}
        <div class="p-3 bg-red-100 border border-red-300 rounded-md text-red-600 text-sm">
          {error}
        </div>
      {/if}
      
      {#if response}
        <div class="mt-6 p-4 rounded-md border border-blue-300 bg-white shadow-md">
          <div class="blu text-blue-600 font-medium max-w-none">
            {@html renderedResponse}
          </div>
        </div>
      {/if}
    </div>
  </div>
</div>

<style>
:global(.blu) {
  color: #3b82f6; 
  font-weight: 700;
}
</style>

